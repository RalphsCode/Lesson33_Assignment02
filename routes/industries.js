...
router.get("/", async function (req, res, next) {
    try { 
        ...
        for (indCode in industryList) {
            
        }
        // const companyResults = await db.query( 'SELECT i.code, i.industry, c.name FROM industries AS i LEFT JOIN company_industry AS c_i ON i.code = c_i.industry_code LEFT JOIN companies AS c ON c.code = c_i.company_code WHERE i.code = $1', [industry_code]); 
